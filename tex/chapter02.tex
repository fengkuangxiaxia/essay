...
\begin{figure}[!htp]
	\centering
	\includegraphics[width=\textwidth]{chap2/hardwareTrivium.png}
...
从上述对Trivium算法的整数性分析，可以发现标准Trivium流密码算法的特征多项可以在模2域上因式分解，并可以得到下列等式：
\[\begin{split}
...
\begin{algorithmic}
  \FOR {$i \leftarrow 1$ \TO $N$}
    ...
    \STATE {$Z  \leftarrow T_{1} + T_{2 } + T_{3}$}
...
算法（\ref{algo:trivium_maple}）中的9个嵌套的For循环将会遍历1到14中所有的9个参数的取值，由于有$\omega_{1}<\omega_{2}<\cdots<\omega_{9}$这一约束条件，所以其计算量会缩减为$6^9$个处理级别，是一个可以接受的范围，因为以本文作者普通的笔记本电脑（CPU：i5 2410，内存 4GB）的运算能力运行时间接近1800秒。最后，Maple运算的结果如下表所示：
